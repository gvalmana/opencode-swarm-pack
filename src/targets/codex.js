...
const { installDirectory, installGeneratedFile } = require("../filesystem");
...
function buildDeveloperInstructions(agentName, body) {
  const adaptedBody = body.replace(
    /Use the `opencode-swarm` skill\./g,
    "Use the opencode-swarm skill when available; otherwise follow the embedded swarm discipline and handoff rules directly."
  );

  return [
    "These instructions were adapted from the OpenCode swarm role definition for Codex custom agents.",
...
function installSkills(teamDirectory, options) {
  const sourceDirectory = path.join(teamDirectory, "skills");
  if (!fs.existsSync(sourceDirectory) || !fs.statSync(sourceDirectory).isDirectory()) {
    return;
  }

  installDirectory(sourceDirectory, resolveSkillDirectory(options), options);
}
...
- Use the opencode-swarm skill when available with \`$opencode-swarm\`.
